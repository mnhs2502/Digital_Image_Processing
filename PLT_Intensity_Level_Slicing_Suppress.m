%% MATLAB code: Gray Level (Intensity Level) Slicing Transformation (Supressing Others)
...
for i = 1:row
    for j = 1:col
        if (GrayImage(i,j) > T1) && (GrayImage(i,j) < T2)
            StretchedImage(i,j) = 255;  % Highlight pixels within the range
        else
            StretchedImage(i,j) = 0;    % Suppress others
        end
    end
end
...
figure
montage({GrayImage, StretchedImage}, 'Size', [1 2]);
